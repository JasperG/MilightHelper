...
public class Milight
{
	...
	private class MilightDevice implements Comparable<MilightDevice>
	{
	...
	public void discover()
	{
		...
		try
		{
			...
			for (NetworkInterface networkInterface : Collections.list(NetworkInterface.getNetworkInterfaces()))
			{
				...
				for (InterfaceAddress interfaceAddress : networkInterface.getInterfaceAddresses())
				{
					...
					System.out.println("Attempting to discover Milight devices on Network Interface " + networkInterfaceName);
					...
					if (milightDevices.size() > 0)
					{
						...
						/* We have lift-off! Ten minste één milight apparaat gevonden */

						Collections.sort(milightDevices);

						System.out.println("Attempting connection to " + milightDevices.get(0).addrIP + " on " + networkInterfaceName);
		...
		/* No connection has been made, close socket */
		if (socket != null) socket.close();

		if (triedInterfaces == 0)
		{
			System.out.println("No suitable IPv4 network interfaces found.");
		}
		else if (milightDevices.size() == 0)
		{
			System.out.println("No WiFi iBox1 ( v6 ) was found on any of your network interfaces ( No device responded within the set timeout )");
		}
	...
	private void discoverDevices(InetAddress localAddress, InetAddress broadcastAddress)
	{
		...
		{
			System.out.println("Could not bind to port " + Integer.toString(localPort) + " at " + localAddress.getHostAddress() + " on " + networkInterfaceName);
			return;
		}
		...
		do
		{
			...
			try
			{
				System.out.println("Sending discovery packet over " + networkInterfaceName);
			...
			discoveryLoop: do
			{
				...
				try
				{
					...
					if (discovery.length == 3 && discovery[1].length() == 12)
					{
						...
						System.out.println("Got a response from " + discovery[0] + " on " + networkInterfaceName);
	...
	private void setDevice(String address)
	{
		...
		if (!isConnected)
		{
			...
			System.out.println("The Milight device at " + milightAddress.getHostAddress() + " did not respond upon attempting to establish a session");

		}
		else
		{
			System.out.println("Now ready to communicate with Milight device at " + milightAddress.getHostAddress() + " on " + networkInterfaceName);
	...
	private void passwordDiscovery()
	{
					...
					try
					{
						...
						if (packet.getAddress().equals(milightAddress))
						{
							...
							if (response.indexOf("8000000015") == 0 || response.indexOf("8000000021") == 0)
							{
								...
								System.out.println("The Milight device at " + milightAddress.getHostAddress() + " revealed it's password to be " + milightPassword);
		...
		System.out.println("The Milight device at " + milightAddress.getHostAddress() + " did not return it's password");
	...
	private void startWorkerThread()
	{
		...
		new Thread() {
			...
			public void run()
			{
				...
				if (socket != null) socket.close();
				System.out.println("Worker thread died.");
	...
	{
		/* Setting isConnected to false will cause the worker thread to die */
		isConnected = false;
	}
